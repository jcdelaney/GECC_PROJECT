function result = animal_go_no_go(subject, session, varargin)
% Usage:
%     >> animal_go_no_go(subject, session) % run the study
%     OR
%     >> animal_go_no_go(subject, session, optional) % run the study in practice mode or debug mode
%
% Parameters:
...
global LIGHT_GRAY % define light gray color
...
RESP_DURATION =     1.5;% duration of the response for each trial
...
RECT = CenterRectOnPoint([0,0,800,450],RESOLUTION(1)/2,RESOLUTION(2)/2); % background rectangle
...
DirName = ['GO_',SUBJECT_STRING,'_',num2str(SESSION)];
...
if DEBUG
    fprintf('[GO] This is where I would get Screen information\n');
    fprintf('[GO] Getting flip interval\n');
    START_TIME = cputime;
    WINDOW_RECT = [-1 -1 -1 -1];
else
...
if DEBUG
    fprintf('%s\n',sprintf('[GO] Showing welcome screen:\n+--\n%s\n+--\n',WELCOME_TEXT));
else
    try
    ...
    while inprogress
        ...
        for run=StartRun:length(TrialList)
            ...
            try
                String = 'Please wait...';
                if DEBUG
                    fprintf('[GO] Showing wait screen...(waiting for user ''q'')...\n');
                    fprintf('+---\n%s\n+---\n',String);
                    %pause(.5);
                    secs = cputime;
                    fprintf('[GO] simulating ''q'' press...\n');
                else
            ...
            try
                if DEBUG
                    fprintf('[GO] Waiting for scanner input (''s'')...\n');
                    fprintf('+---\n%s\n+---\n',String);
                    %pause(.5);
                    secs = cputime;
                    fprintf('[GO] simulating ''s'' press...\n');
                else
            ...
            for block=StartBlock:length(RUN)
                ...
                % show stimuli
                [success, num_fail_block] = ShowStimuli(BLOCK.trials);
                if PRACTICE
                    num_fail = num_fail + num_fail_block;
                end
                if ~ success
            ...
            StartBlock = 1;
            if run == floor(length(TrialList)/2)
                [Failed, ErrorMessage] = SendTrigger(EVENT.Pause, GetSecs);
                Screen('DrawingFinished', WINDOW,0);
                Screen('Flip',WINDOW);
                WaitSecs(30);
            end
            ...
            if DEBUG
                ...
                try
                   ...
                   practice_success = (num_fail <= 1);
                   if PRACTICE & practice_success
                       DoneText = ['Practice run complete.\n\nYour score was ',num2str(12-num_fail),' out of 12.\n\nGood job!'];
                   elseif PRACTICE
                       DoneText = ['Practice run complete.\n\nYour score was ',num2str(12-num_fail),' out of 12.\n\nPlease try again.'];
                   else
